...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        if self.custom_eval:
            ...
            self.filenames, self.img_info = load_image_filenames(
                img_dir, image_file)  # length equals to split_mask
    ...
    def __getitem__(self, index):
        ...
        if img.size[0] != self.img_info[index]['width'] or img.size[1] != self.img_info[index]['height']:
            print('=' * 20, ' ERROR index ', str(index), ' ', str(img.size), ' ',
                  str(self.img_info[index]['width']), ' ', str(self.img_info[index]['height']), ' ', '=' * 20)

        flip_img = (random.random() > 0.5) and self.flip_aug and (
            self.split == 'train')
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        target.add_field("attributes", torch.from_numpy(
            self.gt_attributes[index]))
        ...
        for i in range(relation.shape[0]):
            if relation_map[int(relation[i, 0]), int(relation[i, 1])] > 0:
                if (random.random() > 0.5):
                    relation_map[int(relation[i, 0]), int(
                        relation[i, 1])] = int(relation[i, 2])
            else:
                relation_map[int(relation[i, 0]), int(
                    relation[i, 1])] = int(relation[i, 2])
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    ...
    filter_obj_idx = [86, 56, 95, 53, 90, 67, 64, 44, 114,
                      78, 145, 45, 87, 58, 99, 11, 40, 134, 28, 19, 62]
    ...
    for i in range(len(image_index)):
        ...
        if split == 'train':
            ...
            idx_map = {}   # 用于relation中关系的重新映射
            gt_cls = gt_classes_i  # 获得gt_object的标签
            keep = np.ones_like(gt_cls)  # 初始化一个全1向量
            for j, gc in enumerate(gt_cls):  # 对该图片中所有的object进行判断
                if gc in filter_obj_idx:  # 如果在过滤列表中，那么就要进行过滤
                    keep[j] = 0  # 将其对应的位置变成0即可
            keep = keep.astype(np.bool)   # 将其变成一个T/F

            cnt = 0
            for j in range(len(keep)):  # 进行索引映射
                if keep[j] == True:  # 因为之后的relation所基于的还是之前的索引，因此需要将其换成最新的索引
                    idx_map[j] = cnt
                    cnt = cnt + 1

            filter_idx_in_image = np.where(keep == 0)[0]

            boxes_i = boxes_i[keep]
            gt_classes_i = gt_classes_i[keep]
            gt_attributes_i = gt_attributes_i[keep]

            keep_relation = np.ones(rels.shape[0], dtype=np.bool)
            # rels: (n, 3)
            for j, rel in enumerate(rels):
                if (rel[0] in filter_idx_in_image) or (rel[1] in filter_idx_in_image):
                    keep_relation[j] = False

            rels = rels[keep_relation]
            for j in range(rels.shape[0]):
                rels[j][0] = idx_map[rels[j][0]]
                rels[j][1] = idx_map[rels[j][1]]

            if len(gt_classes_i) == 0:
                print(image_index[i])
                split_mask[image_index[i]] = 0
                #ccnt += 1
                continue
        ...
        # gt_classes: list, item: np.array shape (14,)
