...
class RelationSampling(object):
    ...
    def gtbox_relsample(self, proposals, targets):
        ...
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            tgt_sub_rel_matrix = target.get_field("sub_relation") # [tgt, tgt]
            tgt_sub_pair_idxs = torch.nonzero(tgt_sub_rel_matrix > 0)
            assert tgt_sub_pair_idxs.shape[1] == 2
            tgt_sub_head_idxs = tgt_sub_pair_idxs[:, 0].contiguous().view(-1)
            tgt_sub_tail_idxs = tgt_sub_pair_idxs[:, 1].contiguous().view(-1)
            tgt_sub_rel_labs = tgt_sub_rel_matrix[tgt_sub_head_idxs, tgt_sub_tail_idxs].contiguous().view(-1)
            ...
            if tgt_pair_idxs.shape[0] > num_pos_per_img:
                ...
                tgt_pair_idxs = tgt_pair_idxs[perm]
                tgt_rel_labs = tgt_rel_labs[perm]
                tgt_sub_rel_labs = tgt_sub_rel_labs[perm]
            num_fg = min(tgt_pair_idxs.shape[0], num_pos_per_img)
            ...
            img_sub_rel_labels = torch.cat((tgt_sub_rel_labs.long(), torch.zeros(tgt_bg_idxs.shape[0], device=device).long()), dim=0).contiguous().view(-1)

            rel_idx_pairs.append(img_rel_idxs)
            rel_labels.append(img_rel_labels)
            sub_rel_labels.append(img_sub_rel_labels)

        return proposals, rel_labels, rel_idx_pairs, rel_sym_binarys#, sub_rel_labels
