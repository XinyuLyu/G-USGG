...
#                                                                                                              targets)
#         else:
#             rel_labels, rel_binarys, sub_rel_labels = None, None, None
...
#             union_features = None
#
#         # refine_logits, relation_logits, add_losses,rel_rep_norm = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger)
...
#         # return roi_features, proposals, output_losses,rel_rep_norm
#
# # class ROIRelationHead(torch.nn.Module):
# #     """
# #     Generic Relation Head class.
...
# #         #refine_logits, relation_logits, relation_logits_sub,add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger, sub_rel_labels=sub_rel_labels)
# #         refine_logits, relation_logits,add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger, sub_rel_labels=sub_rel_labels)
# #
# #         # for test
# #         if not self.training:
# #             result = self.post_processor((relation_logits[1], refine_logits), rel_pair_idxs, proposals)
# #             return roi_features, result, {}
# #
# #         loss_relation, loss_refine = self.loss_evaluator(proposals, rel_labels, relation_logits,refine_logits, sub_rel_labels)
# #         #loss_relation, loss_refine = self.loss_evaluator(proposals, rel_labels, relation_logits, relation_logits_sub,refine_logits, sub_rel_labels)
# #
# #         if self.cfg.MODEL.ATTRIBUTE_ON and isinstance(loss_refine, (list, tuple)):
# #             output_losses = dict(loss_rel=loss_relation, loss_refine_obj=loss_refine[0], loss_refine_att=loss_refine[1])
# #         else:
