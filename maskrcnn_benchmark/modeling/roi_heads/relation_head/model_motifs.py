...
class LSTMContext(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        self.use_context = self.cfg.MODEL.ROI_RELATION_HEAD.USE_CONTEXT
        self.lin_obj = nn.Linear(4424, self.hidden_dim)
        self.lin_edge = nn.Linear(4808, self.hidden_dim)
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        # todo: disable context for object encoding
        if self.use_context:
            ...
            obj_ctx = self.lin_obj(obj_pre_rep)
            obj_preds = obj_labels
            obj_dists = to_onehot(obj_preds, self.num_obj_classes)
...
class LSTMContext_mp(nn.Module):
    """
    Modified from neural-motifs to encode contexts for each objects
    """
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        super(LSTMContext_mp, self).__init__()
        self.cfg = config
        self.obj_classes = obj_classes
        self.rel_classes = rel_classes
        self.num_obj_classes = len(obj_classes)
        ...
        self.post_emb = nn.Linear(4808, self.mlp_dim * 2)
        self.linear_edge = nn.Linear(self.mlp_dim, self.hidden_dim)
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        # abandone context for object encoding
        obj_ctx = self.lin_obj(obj_pre_rep)
        obj_preds = obj_labels
        obj_dists = to_onehot(obj_preds, self.num_obj_classes)
