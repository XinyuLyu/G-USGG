...
# from .model_transformer import TransformerContext_mp
# from .model_vctree import VCTreeLSTMContext_mp
...
#
# @registry.ROI_RELATION_PREDICTOR.register("Transformer_sub_prototype_Memory")
# class Transformer_sub_prototype_Memory(nn.Module):
#     def __init__(self, config, in_channels):
#         super(Transformer_sub_prototype_Memory, self).__init__()
#         self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
#         self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
#
#         self.cfg = config
...
#         assert self.num_rel_cls == len(rel_classes)
#         # init contextual lstm encoding
#         self.context_layer = TransformerContext_mp(config, obj_classes, rel_classes, in_channels)
#
#         # post decoding
#         self.mlp_dim = 4096
#         self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM
...
#                     m_pair = m_rel_pair.to("cuda")
#                     m_union = m_union.to("cuda")
#                     _, _, edge_ctx = self.context_layer(m_roi, m_proposal_list)
#
#                     mem_edge_rep = self.post_emb(edge_ctx) # [N, 2048]
#                     mem_edge_rep = mem_edge_rep.view(mem_edge_rep.size(0), 2, self.mlp_dim // 2)
#                     mem_head_rep = mem_edge_rep[:, 0].contiguous().view(-1, self.mlp_dim // 2) # [N_obj, 1024] 都表示 obj 的 repr
...
#
#         # extract feature ------ encode context infomation (extract relation feature from object)
#         obj_dists, obj_preds, edge_ctx = self.context_layer(roi_features, proposals)  # edge_ctx 代表混合了 visual 和 embedding 特征的 predicate 特征
#
#         # post decode
#         edge_rep = self.post_emb(edge_ctx)  # [N_obj, 2, 1024]
#         edge_rep = edge_rep.view(edge_rep.size(0), 2, self.mlp_dim // 2)
...
#             attn_mem_rep, _ = self.attn(rel_rep.unsqueeze(0), mem_rel_reps.unsqueeze(0), mem_rel_reps.unsqueeze(0))
#             fusion_mem_rep = F.normalize(attn_mem_rep.squeeze(0),dim=0)
#             rel_rep = 0.99 * rel_rep + 0.01 * fusion_mem_rep
#
#         predicate_proto = self.W_pred(self.predicate_proto)  # [307, 2048]
#         predicate_proto_sub = self.W_pred(self.codebook._C)
#
...
#         return obj_dists, rel_dists, add_losses
#
# @registry.ROI_RELATION_PREDICTOR.register("VCTree_sub_prototype_Memory")
# class VCTree_sub_prototype_Memory(nn.Module):
#     def __init__(self, config, in_channels):
#         super(VCTree_sub_prototype_Memory, self).__init__()
#         self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
#         self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
#
#         self.cfg = config
...
#         assert self.num_rel_cls == len(rel_classes)
#         # init contextual lstm encoding
#         self.context_layer = VCTreeLSTMContext_mp(config, obj_classes, rel_classes, statistics, in_channels)
#
#         # post decoding
#         self.mlp_dim = 4096
#         self.hidden_dim = config.MODEL.ROI_RELATION_HEAD.CONTEXT_HIDDEN_DIM
...
#         # load memory
#         self.iters += 1
#         if self.iters >= 0 and self.use_memory:
#             self.is_timing = True
#         #self.is_timing = True
#         if self.is_timing:
#             with torch.no_grad():
...
#                     m_pair = m_rel_pair.to("cuda")
#                     m_union = m_union.to("cuda")
#                     _, _, edge_ctx, _ = self.context_layer(m_roi, m_proposal_list)
#
#                     mem_edge_rep = self.post_emb(edge_ctx) # [N, 2048]
#                     mem_edge_rep = mem_edge_rep.view(mem_edge_rep.size(0), 2, self.mlp_dim // 2)
#                     mem_head_rep = mem_edge_rep[:, 0].contiguous().view(-1, self.mlp_dim // 2) # [N_obj, 1024] 都表示 obj 的 repr
...
#
#         # extract feature ------ encode context infomation (extract relation feature from object)
#         obj_dists, obj_preds, edge_ctx, binary_preds = self.context_layer(roi_features, proposals)  # edge_ctx 代表混合了 visual 和 embedding 特征的 predicate 特征
#
#         # post decode
#         edge_rep = F.relu(self.post_emb(edge_ctx))  # [N_obj, 2, 1024]
#         edge_rep = edge_rep.view(edge_rep.size(0), 2, self.mlp_dim // 2)
#         head_rep = edge_rep[:, 0].contiguous().view(-1, self.mlp_dim // 2)  # [N_obj, 1024] 都表示 obj 的 repr
#         tail_rep = edge_rep[:, 1].contiguous().view(-1, self.mlp_dim // 2)  # [N_obj, 1024] 都表示 obj 的 repr
#
...
#         return obj_dists, rel_dists, add_losses
#
# @registry.ROI_RELATION_PREDICTOR.register("VectorPredictor_mine")
# class VectorPredictor_mine(nn.Module):
#     def __init__(self, config, in_channels):
#         super(VectorPredictor_mine, self).__init__()
#
#         self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
...
#             self.mask_general_sub_matching[start:start + length, k] = 0
#             start = start+length
#         self.context_layer = VectorFeature(config, obj_classes, rel_classes, in_channels)
